...
export default async function handler(req: DraftkingsRequest, res: NextApiResponse) {
  ...
  try {
    ...
    req.body.refs.map(item => {
      console.log(item[1] + 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
      transactionArray.push(item[1])
    })
    ...
    //infoArray.push(header)
      ...
      for (let i = 0; i < transactionArray.length; i++) {
          ...
          await page.waitForSelector('input[name="transactionId"]')
          await page.type('input[name="transactionId"]', ref)
          return
          ...
          const transactionId = await page.waitForSelector(
            'input[name="transactionId"]'
          )

          await transactionId.click({ clickCount: 3 })
          await transactionId.press('Backspace')
